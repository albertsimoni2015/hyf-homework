...
insert into task (id, title, description, created, updated, due_date, status_id, user_id) values (23, 'Buy new phone', 'The battery in the current one only lasts 5 hours :disappointed:', '2017-09-17 00:25:34', '2017-10-09 11:48:12', null, 3, NULL);
...
insert into task (id, title, description, created, updated, due_date, status_id, user_id) values (29, 'Sign up for linkedin', 'Make the CV awesome! :smile:', '2017-09-04 00:57:47', '2017-10-18 18:07:48', '2017-12-07 23:04:38', 3, 2);
...
SELECT * FROM task WHERE due_date='NULL';
...
SELECT title , due_date  FROM task WHERE title OR description LIKE '%database%'
-- Get the title and status (as text) of all tasks
SELECT title AS title_Activity ,
 status AS status_Funtion FROM task 
JOIN status ON status.id = task.status_id;
-- Get the name of each status, along with a count of how many tasks have that status
SELECT status AS status_task .
 COUNT(task.tittle)
FROM task
Join status ON staus.id = task.status_id GROUP BY status_id;
